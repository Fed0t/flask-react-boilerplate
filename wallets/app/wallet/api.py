...
class WalletAPI:
    ...
    @staticmethod
    def load_wallet(configuration, private_key, passphrase):
        """
        Create new wallet account and save encrypted keystore
        :param configuration: loaded configuration file instance
        :param password: set password from keystore and used also as entropy for CSPRNG
        :return: created wallet object and saved keystore path
        """
        wallet = Wallet(configuration).create_with_private(private_key)
    ...
    def send_transaction(
        ...
        value,
        token_symbol=None,
        gas_price_speed=10,  # MetaMask default transaction speedup is gasPrice*10
    ):
        """
    ...
    def add_contract(configuration, contract_symbol, contract_address):
        ...
        contract = Contract(configuration, checksum_address)
        contract.add_new_contract(contract_symbol, checksum_address)

    @staticmethod
    def remove_contract(configuration, contract_symbol):
        """
        Adds new contract ERC20 token into config file with symbol and address
        :param configuration: configuration file
        :param contract_symbol: contract symbol
        :return:
        """
        configuration.remove_contract_token(contract_symbol)
    ...
    def list_tokens(configuration):
        ...
        """
        if "NCC" in configuration.contracts:
            configuration.remove_contract_token("NCC")
